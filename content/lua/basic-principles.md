...
darktable treats two kinds of locations differently: files that are _executed_ as Lua code, and directories that are _scanned_ for scripts:

* `luarc`-files executed at startup:
    * `$DARKTABLE/share/darktable/luarc`: run by darktable itself. This is the entry point that launches the (bundled) script manager. It serves internal purposes only and should not be edited. $DARKTABLE is the program's installation directory.
    * `[configuration-directory]/luarc`: If present -- this is one way of calling your own scripts (see below).
