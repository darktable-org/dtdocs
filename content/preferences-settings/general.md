...
interface language
: Set the language of the user interface. It includes the option to enable sentence case (`en@truecase`) for `en` (currently the `US` variant, and may not apply to subsequent variants). The system default is marked with an * (needs a restart)
