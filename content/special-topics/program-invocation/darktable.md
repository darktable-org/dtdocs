...
: darktable keeps a cache of image thumbnails for fast image preview and precompiled OpenCL binaries for fast startup. Multiple thumbnail caches may exist in parallel -- one for each library file. The default cache path is listed on the [configuration directory](../../preferences-settings/config-directory.md#cache-directory) page.
