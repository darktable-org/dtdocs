...
darktable stores its edits in two places: 
  ...
  - XMP sidecar files: `image.raw.xmp`; if enabled in the [preferences](../preferences/storage.md)
  - `library.db` (unless using `--library :memory:`-mode)

Presets, settings etc. are stored in further files inside the [configuration directory](../preferences/config-directory.md).
...
Setup:
* Use a file-synchronization service of your choice (e.g. Syncthing, Dropbox etc.) to sync the RAW-Files + XMP files between your devices.
...
## Approach B: Sync files + entire config dir

More complicated to setup, more error prone and requires some special considerations. 

Setup:

* Use a file-synchronization service of your choice (e.g. Syncthing; e.g. Dropbox might not work as well because it doesn't allow for syncing of multiple folders in different locations) to sync the RAW-Files + XMP files AND the contents of your [configuration directory](../preferences-settings/config-directory.md) between your devices.
...
* Machine specific settings could get overridden like OpenCL settings.
* Consider excluding the cache directory from sync. `~/.cache/darktable` (thumbnails etc.) is large and fully regenerable, syncing it just costs bandwidth and storage for no benefit.
* Consider a robust versioned backup concept.
