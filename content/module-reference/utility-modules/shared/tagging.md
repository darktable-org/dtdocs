...
: You can use tag names to define [collections](./collections.md) that are displayed, processed, and exported in a _durable_ [custom sort](./collection-filters.md#custom-sort) order, which does not change when other collections (containing some of the same images) are reordered.
