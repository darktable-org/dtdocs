...
All of this module's adjustments are performed in the color space defined by the _adaptation_ control on the CAT tab. The module can be made into a "pure" channel mixer (without white balance) in pipeline RGB color space by setting the _adaptation_ to "none (bypass)". To perform channel mixing in one of the _adaptation_ color spaces without white balancing, set the _illuminant_ to "same as pipeline (D50)".

# Chromatic Adaptation Transformation (white balance)

Chromatic adaptation aims to predict how all surfaces in the scene would look if they had been lit by another illuminant. What we actually want to predict, though, is how those surfaces would have looked like if they had been lit by the same illuminant as your monitor. White balance on the other hand aims only at ensuring whites are really whites, that is R = G = B, and doesn’t really care about the rest of the color range.

Chromatic adaptation functionality is controlled within the Chromatic Adaptation Transformation (CAT) tab of the _color calibration_ module. When used in this way the _white balance_ module only needs to perform a basic white balance operation assuming a D65 illuminant ("camera reference" mode). The remainder of the white balance adaptation is then performed by the _color calibration_ module. 
...
By default, _color calibration_ will:

- read the raw EXIF to fetch the scene white balance set by the camera
- adjust this setting with the reference white balance used in the white balance module
- adjust this setting again with the input color profile in use (only the standard matrix)
...
The CAT modes in this module rely on a number of assumptions in order to work correctly and it can be easy to inadvertently break these assumptions in subtle ways. To help you to avoid these kind of mistakes, the _color calibration_ module will show warnings in the following circumstances.
...
The white balance modes in _color balance_ can be disabled by either setting the _adaptation_ to "none (bypass)" or setting the _illuminant_ to "same as pipeline (D50)" in the CAT tab.
...
: - _same as pipeline (D50)_: Do not perform white balancing in this module instance but perform channel mixing using the selected _adaptation_ color space.
...
: - _custom_: If a neutral grey patch is available in the image, the color of the illuminant can be selected using the color picker, or can be manually specified using hue and saturation sliders (in LCh perceptual color space). The color swatch next to the color picker shows the color of the calculated illuminant (the light that was used to illuminate the scene) that will be used in the CAT compensation. The color picker can also be used to restrict the area used for AI detection (below).
