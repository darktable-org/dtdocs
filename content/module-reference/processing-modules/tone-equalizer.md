...
preserve details
: Choose the smoothing algorithm to use when blurring the mask. A guided filter will be used by default, which is effective at preserving strong edges while avoiding gradient reversals that result in artifacts like halos. 
 - _no_ means that no smoothing of the mask is applied. This can cause local contrast in the image to be compressed.
 - _guided filter_ uses the original guided filter algorithm to blur the mask while attmepting to preserve edges. One of the limitations of this algorithm is that the guided filter is exposure-sensitive, meaning that shadows will experience a lot more blurring than the highlights.
 - _average guided filter_ is used in cases where the effect of the guided filter is too strong. In this mode, a geometric mean is taken between the output of the original _guided filter_ algorithm and the output that is given by the _no_ option. 
 - _eigf (default)_ the _exposure independant guided filter_ solves the problem of te original guided filter, in that it makes the degree of blurring independant of the exposure level. This means the degree of blurring applied in the mask to both the highlights and the shadows regions should be about the same. This improved algorithm has now become the default option.
 - _averaged eigf_ takes the geomteric mean between the _eigf_ mask and the mask generated by the _no_ option, and is useful in cases where the degree of blurring in the mask needs to be mitigated.
