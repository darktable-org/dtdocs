...
: Choose the smoothing algorithm to use when blurring the mask. An exposure independent guided filter will be used by default, which is effective at preserving strong edges while avoiding gradient reversals that result in artifacts like halos. 
 ...
 - _guided filter:_ Use the original guided filter algorithm to blur the mask while attempting to preserve edges. One of the limitations of this algorithm is that the guided filter is exposure-sensitive, meaning that shadows will experience a lot more blurring than the highlights. Note that this limitation can be an asset sometimes: if one wants to lighten shadows a lot, guided filter can provide a very good local contrast preservation.
...
## presets
The _tone equalizer_ comes with several presets that use either the exposure independent guided filter (eigf), either the guided filter (gf). The presets that use guided filter better preserve local contrast in shadows than presets that use eigf, at the price of destroying more the local contrast in the highlights. Depending on your photo, one or the other can lead to better results. These presets preserve middle-grey.
